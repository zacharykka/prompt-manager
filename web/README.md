...
- **网络层**：原生 fetch + 拦截器封装，或 axios（配合 Token 注入与错误处理）。
...
2. **Dashboard/PromptList**：
   - 搜索框、过滤 Tag、分页控制。
   - 列表项支持查看、激活、复制、删除等快捷操作。
3. **PromptDetail**：
   - 信息卡片展示基础属性。
   - Tab 切换 "版本历史"、"统计"、"审计" 等面板。
4. **PromptEditor**：
   - Modal 或 Drawer，含基础信息、变量说明、内容编辑器（Markdown/富文本）。
   - 表单校验、保存草稿、提交回调。
5. **VersionHistory**：
...
4. **Prompt 列表迭代**：实现列表视图、搜索过滤、分页、空态/加载态。
